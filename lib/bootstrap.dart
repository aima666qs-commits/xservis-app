...
Future<void> lazyBootstrap(WidgetsBinding widgetsBinding, Environment env) async {
  ...
  final appInfo =
      await _init('app info', () => container.read(appInfoProvider.future));
  await _init(
    'preferences',
    () => container.read(sharedPreferencesProvider.future),
  );

  // AIMA builds keep analytics permanently disabled. The provider writes the
  // privacy-safe preference and never initializes a remote telemetry SDK.
  await _init(
    'privacy mode',
  ...
  await _init(
    ...
    () => container.read(
      chainProfileNotifierProvider(ChainType.extraSecurity).future,
    ),
  );
  await _init(
    'chain profile unblocker',
    () => container.read(
      chainProfileNotifierProvider(ChainType.unblocker).future,
    ),
  );
  await _safeInit(
    'hiddify-core',
  ...
  Logger.bootstrap.info(
    'bootstrap took [${stopWatch.elapsedMilliseconds}ms]',
  );
  stopWatch.stop();
